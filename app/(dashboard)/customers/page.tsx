...
import { Plus, Search, Edit, Trash2, X, Users, History, Phone, MapPin, DollarSign, Wrench, Printer, ExternalLink, Loader2 } from 'lucide-react';
...
export default function CustomersPage() {
      ...
      if (cData) {
        cData.forEach(c => {
          allCustomersMap.set(c.id, {
            ...
            phone: c.phone || '',
            address: c.address || '',
            joinedDate: c.created_at ? new Date(c.created_at).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }) : ''
          });
        });
      }
      ...
      if (rData) {
        setRepairsData(rData.map((repair: any) => {
          ...
          if (!repair.customer_id && customerName) {
            const existingCust = Array.from(allCustomersMap.values()).find(c => c.phone === customerPhone || c.name.toLowerCase() === customerName.toLowerCase());
            if (!existingCust) {
              ...
              allCustomersMap.set(tempId, {
                ...
                phone: customerPhone,
                address: '',
                joinedDate: repair.created_at ? new Date(repair.created_at).toLocaleDateString('en-US', { month: 'short', day: '2-digit', year: 'numeric' }) : ''
              });
            }
          }
  ...
  const getCustomerStats = (customer: Customer) => {
    ...
    const cSales = salesData.filter(s => 
    ...
    const cRepairs = repairsData.filter(r => 
    ...
    return {
      ...
      historyLog: [...cSales.map(s => ({ ...s, type: 'Sale' })), ...cRepairs.map(r => ({ ...r, type: 'Repair' }))]
  ...
  const printSaleInvoice = async (sale: any) => {
    ...
    const rowsHtml = itemsList.map((item: any, index: number) => {
      const total = (parseFloat(item.price) || 0) * (parseInt(item.qty) || 1);
      ...
      <div className="bg-white border border-gray-200 rounded-xl shadow-sm overflow-hidden flex flex-col min-h-[300px]">
        {isLoading ? (
          <div className="flex flex-col items-center justify-center flex-1 py-12">
            <Loader2 className="animate-spin text-[#1a9e52] mb-4" size={40} />
            <p className="text-gray-500 font-medium">កំពុងទាញយកទិន្នន័យពី Database...</p>
          </div>
        ) : (
                    ...
                    <tr key={customer.id} className="hover:bg-gray-50 transition-colors">
                      <td className="px-6 py-4">
                        ...
                        {customer.id.startsWith('temp-') && (
                          <span className="inline-block mt-1 px-2 py-0.5 bg-amber-100 text-amber-700 text-[10px] rounded font-bold uppercase tracking-wider">Repair</span>
                        )}
                      </td>
                      <td className="px-6 py-4">
                      ...
                      <td className="px-6 py-4 text-center">
                        <div className="flex items-center justify-center gap-3 text-gray-400">
                          ...
                          <button onClick={() => handleEdit(customer)} className="hover:text-blue-600 transition-colors" title={customer.id.startsWith('temp-') ? 'រក្សាទុកអតិថិជននេះទៅ Database' : 'កែប្រែ'}>
                            <Edit size={18} />
                          </button>
          ...
          <div className="bg-white rounded-xl shadow-xl w-full max-w-md overflow-hidden">
            ...
            <form onSubmit={handleSave} className="p-6 space-y-4">
              ...
              <div className="pt-4 flex gap-3">
                ...
                <button disabled={isSaving} type="submit" className="flex-1 px-4 py-2.5 bg-[#1a9e52] text-white rounded-lg text-sm font-medium hover:bg-emerald-600 shadow-sm flex justify-center items-center gap-2">
                    ...
                    {stats.historyLog.map((log: any, idx: number) => (
                      <div key={idx} className="border border-gray-100 rounded-xl p-4 hover:bg-gray-50 transition-colors">
                        ...
                        <p className="text-xs text-gray-500 mt-1">
                          {log.type === 'Repair' ? (log.items ? log.items.map((i:any)=>i.description).join(', ') : log.issue) : 'វិក្កយបត្រ #' + (log.id.length > 8 ? log.id.substring(0,8).toUpperCase() : log.id)}
                        </p>
