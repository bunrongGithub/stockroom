...
import { supabase } from '@/supabase/supabase'; // Changed back to correct alias
import {
    Edit,
    Loader2,
    Plus,
...
export default function SalesPage() {
    ...
    const adjustStock = async (itemsToAdjust: any[], isAdding: boolean) => {
        ...
        for (const saleItem of itemsToAdjust) {
            ...
            try {
                ...
                console.error(
                    'Error adjusting stock for item:',
    ...
    const handleSaveSale = async () => {
        if (
            !newSaleCustomer ||
            newSaleItems.some((i) => !i.description || !i.price)
        ) {
        ...
        try {
            ...
            const oldSale = editingSaleId
                ? salesData.find((sale) => sale.id === editingSaleId)
                : null;

            if (oldSale && oldSale.status !== 'Refunded') {
                await adjustStock(
                    oldSale.items || [{ description: oldSale.model, qty: 1 }],
                    true,
                ); // បូកស្តុកចាស់ចូលវិញសិន
            }
            ...
            const salePayload = {
                ...
                description:
                    newSaleItems[0].description +
                    (newSaleItems.length > 1
                        ? ` (+${newSaleItems.length - 1} មុខទៀត)`
                        : ''),
    ...
    const handleEditSale = (sale: any) => {
        ...
        setNewSaleDiscountValue(
            sale.discountValue ? sale.discountValue.toString() : '',
        );
        setNewSaleDiscountType(sale.discountType || 'fixed');
        setNewSaleWarranty(
            sale.warranty !== undefined ? sale.warranty.toString() : '1',
        );
        ...
        if (window.confirm('តើអ្នកពិតជាចង់លុបទិន្នន័យនេះមែនទេ?')) {
            ...
            try {
                if (saleToDelete && saleToDelete.status !== 'Refunded') {
                    await adjustStock(
                        saleToDelete.items || [
                            { description: saleToDelete.model, qty: 1 },
                        ],
                        true,
                    ); // បូកស្តុកចូលវិញក្នុង Database
                }
    ...
    const handleRefund = async (sale: any) => {
        if (sale.status === 'Refunded')
            return alert('វិក្កយបត្រនេះត្រូវបាន Refund រួចរាល់ហើយ!');
        ...
        if (warrantyMonths > 0) {
            ...
            expDateStr = d.toLocaleDateString('en-US', {
                month: 'short',
                day: '2-digit',
                year: 'numeric',
            });
        ...
        if (warrantyMonths === 0)
            confirmMsg = `ទំនិញនេះគ្មានការធានាទេ! តើអ្នកពិតជាចង់ Refund ប្រាក់ $${finalAmount} មែនទេ? (ស្តុកនឹងត្រូវបានបូកចូលឃ្លាំងវិញ)`;
        else if (isUnderWarranty)
            confirmMsg = `ទំនិញនេះនៅមានធានាដល់ថ្ងៃទី ${expDateStr}។ តើអ្នកចង់ Refund ប្រាក់ $${finalAmount} មែនទេ? (ស្តុកនឹងត្រូវបានបូកចូលឃ្លាំងវិញ)`;
        else
            confirmMsg = `⚠️ ប្រយ័ត្ន: ទំនិញនេះបានផុតកំណត់ធានាតាំងពីថ្ងៃទី ${expDateStr}! តើអ្នកនៅតែចង់អនុញ្ញាតការ Refund នេះមែនទេ?`;

        if (window.confirm(confirmMsg)) {
            try {
                await adjustStock(
                    sale.items || [{ description: sale.model, qty: 1 }],
                    true,
                ); // បូកស្តុកចូលវិញពេល Refund
    ...
    const handleWarrantyClaim = async (sale: any) => {
        if (sale.status === 'Warranty Claimed')
            return alert('ទំនិញនេះកំពុងស្ថិតក្នុងការទាមទារធានារួចហើយ!');
        if (sale.status === 'Refunded')
            return alert(
                'មិនអាចទាមទារធានាបានទេ ព្រោះវិក្កយបត្រនេះបាន Refund រួចហើយ!',
            );

        const warrantyMonths = parseInt(sale.warranty) || 0;
        if (warrantyMonths === 0)
            return alert('ទំនិញនេះគ្មានការធានា (No Warranty) តាំងពីដើមមកទេ!');

        const d = new Date(sale.rawDate || sale.date);
        d.setMonth(d.getMonth() + warrantyMonths);
        const expDateStr = d.toLocaleDateString('en-US', {
            month: 'short',
            day: '2-digit',
            year: 'numeric',
        });
        ...
        if (new Date() > d)
            return alert(
                `សុំទោស! ទំនិញនេះបានផុតកំណត់ធានាតាំងពីថ្ងៃទី ${expDateStr} មកម្ល៉េះ។`,
            );

        if (
            window.confirm(
                `✅ ទំនិញនេះនៅមានធានាដល់ថ្ងៃទី ${expDateStr}。\nតើអ្នកពិតជាចង់កត់ត្រាការ "ទាមទារធានា (Warranty Claim)" សម្រាប់ទំនិញនេះមែនទេ?\n(បញ្ជាក់៖ ការធ្វើបែបនេះនឹងមិនកាត់ប្រាក់ចំណូលសរុបទេ)`,
            )
        ) {
            try {
                const { error } = await supabase
                    .from('sales')
                    .update({ status: 'Warranty Claimed' })
                    .eq('id', sale.id);
                if (error) throw error;
                await fetchSales();
                alert(
                    'បានកត់ត្រាការទាមទារធានាជោគជ័យ! សូមចូលទៅទំព័រ Repairs (ជួសជុល) ហើយបង្កើតប្រតិបត្តិការថ្មីក្នុងតម្លៃ $0 ដើម្បីតាមដានដំណើរការបន្ត។',
                );
            } catch (err) {
    ...
    const handlePrintInvoice = (sale: any) => {
        ...
        const settings = {
            ...
            thankYouNote:
                finalSettings.thank_you_note || defaultSettings.thank_you_note,
            warrantyTerms:
                finalSettings.warranty_terms || defaultSettings.warranty_terms,
        };
    ...
    const filteredSales = salesData.filter((sale) => {
        const customerStr = sale.customer
        ...
        return (
            ...
            saleNoStr.includes(query) ||
            `sl-${saleNoStr}`.includes(query)
        );
    });
            ...
            <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
                <div>
                    ...
                    <p className="text-gray-500 mt-1">
                        Manage your phone sales records
                    </p>
                </div>
                <button
                        ...
                        <tbody className="divide-y divide-gray-100">
                            {isLoading ? (
                                <tr>
                                    ...
                                    >
                                        <Loader2
                                            className="animate-spin mx-auto mb-2 text-[#1a9e52]"
                                            size={24}
                                        />
                                        កំពុងទាញទិន្នន័យ...
                                    </td>
                                ...
                                filteredSales.map((sale) => (
                                    ...
                                    >
                                        <td className="px-6 py-4 text-sm font-mono font-bold text-[#1a9e52]">
                                            {sale.saleNo
                                                ? `sl-${sale.saleNo}`
                                                : `sl-${sale.id.length > 8 ? sale.id.substring(0, 8).toUpperCase() : sale.id}`}
                                        </td>
                                        ...
                                        <td className="px-6 py-4 text-sm text-center">
                                            <div className="flex items-center justify-center gap-3 text-gray-400">
                                                ...
                                                <button
                                                    ...
                                                    title="ទាមទារធានា (Claim Warranty)"
                                                >
                                                    <ShieldCheck size={18} />
                                                </button>
                                                <button
                    ...
                    <div className="bg-white rounded-xl shadow-xl w-full max-w-2xl overflow-hidden my-auto">
                        <div className="flex justify-between items-center px-6 py-4 border-b border-gray-100">
                            <h2 className="text-xl font-bold text-gray-900">
                                {editingSaleId
                                    ? 'កែប្រែប្រតិបត្តិការ (Edit Sale)'
                                    : 'បង្កើតវិក្កយបត្រថ្មី (New Sale)'}
                            </h2>
                            <button
                                onClick={() => resetForm()}
                        ...
                        <div className="p-6 space-y-5">
                            <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                                <div>
                                    ...
                                    <div className="flex">
                                        <span className="inline-flex items-center px-3 rounded-l-lg border border-r-0 border-gray-200 bg-gray-50 text-gray-500 font-medium text-sm">
                                            sl-
                                        </span>
                                        <input
                                            type="text"
                                            value={newSaleNo}
                                            onChange={(e) =>
                                                setNewSaleNo(e.target.value)
                                            }
                                            className="w-full border rounded-r-lg px-3 py-2 outline-none focus:ring-2 focus:ring-[#1a9e52] font-mono text-[#1a9e52] font-bold"
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        onChange={(e) =>
                                            setNewSaleCustomer(e.target.value)
                                        }
                                        className="w-full border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-[#1a9e52]"
                                ...
                                <div>
                                    ...
                                    <input
                                        ...
                                        value={newSalePhone}
                                        onChange={(e) =>
                                            setNewSalePhone(e.target.value)
                                        }
                                        className="w-full border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-[#1a9e52]"
                            ...
                            <div>
                                ...
                                <select
                                    value={newSaleStatus}
                                    onChange={(e) =>
                                        setNewSaleStatus(e.target.value)
                                    }
                                    className="w-full border rounded-lg px-3 py-2 outline-none focus:ring-2 focus:ring-[#1a9e52] bg-white"
                                >
                        ...
                        <div className="px-6 py-4 border-t border-gray-100 flex justify-end gap-3 bg-gray-50">
                            ...
                            <button
                                ...
                                className="px-4 py-2 bg-[#1a9e52] text-white rounded-lg text-sm font-medium hover:bg-emerald-600 shadow-sm flex items-center gap-2"
                            >
                                {isSaving ? (
                                    <Loader2
                                        size={16}
