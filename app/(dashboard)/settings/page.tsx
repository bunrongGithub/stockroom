...
import { Settings, Store, Phone, Save, CheckCircle2, Loader2, Image as ImageIcon, FileText } from 'lucide-react';
...
export default function SettingsPage() {
  ...
  const handleSave = async (e: React.FormEvent) => {
    ...
    try {
      ...
      if (!errorMessage || errorDetails === '{}') {
        errorMessage = 'មិនអាចរក្សាទុកបានទេ។ សូមពិនិត្យមើល RLS របស់អ្នក។';
      }
  ...
  const handleLogoUpload = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    try {
      ...
      if (!apiKey) {
        throw new Error("សូមបញ្ជាក់ NEXT_PUBLIC_IMGBB_API_KEY នៅក្នុង .env.local របស់អ្នក!");
      }
      ...
      if (data.success) {
        ...
        throw new Error(data.error?.message || "មិនអាចអាប់ឡូតរូបភាពទៅកាន់ Imgbb បានទេ!");
      }
    } catch (error: any) {
    ...
    <div className="max-w-4xl mx-auto p-4 md:p-8 space-y-6 animate-in fade-in duration-500">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          ...
          <p className="text-gray-500 mt-1">រៀបចំ និងកែប្រែព័ត៌មានហាងរបស់អ្នក</p>
        </div>
        ...
        >
          ...
          <span>{isSaving ? 'កំពុងរក្សាទុក...' : isSaved ? 'បានរក្សាទុក' : 'រក្សាទុក'}</span>
        </button>
      </div>
          ...
          <div className="p-6 grid grid-cols-1 md:grid-cols-2 gap-6">
            ...
            <div className="md:col-span-2 flex items-center gap-4 p-4 border rounded-lg bg-gray-50 relative">
              <div className="w-16 h-16 bg-white border rounded-lg shadow-sm flex items-center justify-center overflow-hidden shrink-0 relative">
                {isUploadingLogo ? (
                  <div className="flex flex-col items-center justify-center">
                    <Loader2 className="animate-spin text-[#1a9e52]" size={24} />
                  </div>
                ) : storeSettings.logo ? (
              ...
              <div className="flex-1">
                ...
                />
                {isUploadingLogo && <p className="text-xs text-emerald-600 mt-1">កំពុងអាប់ឡូតរូបភាពទៅកាន់ Imgbb...</p>}
              </div>
            </div>
