...
export default function LoginPage() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    e.preventDefault();
    if (!email || !password) return setError("សូមបញ្ចូលអ៊ីមែល និងលេខសម្ងាត់!");
  ...
  const handleSignUp = async () => {
    if (!email || !password) return setError("សូមបញ្ចូលព័ត៌មានឱ្យគ្រប់ដើម្បីចុះឈ្មោះ!");
    setLoading(true);
    setError(null);
    try {
    ...
    <div className="min-h-screen flex items-center justify-center bg-slate-50 p-4 font-sans text-slate-900">
      <div className="max-w-md w-full space-y-8 bg-white p-10 rounded-3xl shadow-2xl border border-slate-100">
        <div className="text-center">
          ...
          <h2 className="text-4xl font-black text-slate-900 tracking-tighter">iCase Service</h2>
          <p className="mt-3 text-sm text-slate-500">Log in to manage your mobile store</p>
        </div>
        ...
        <form className="mt-8 space-y-6" onSubmit={handleLogin}>
          <div className="space-y-5">
            <div>
              ...
              <div className="relative text-slate-400 focus-within:text-[#1a9e52]">
                <Mail className="absolute left-4 top-3.5" size={20} />
            ...
            <div>
              ...
              <div className="relative text-slate-400 focus-within:text-[#1a9e52]">
                <Lock className="absolute left-4 top-3.5" size={20} />
          ...
          <div className="flex flex-col gap-3">
            ...
            >
              {loading ? <Loader2 className="animate-spin" /> : "ចូលប្រើប្រាស់"}
            </button>
