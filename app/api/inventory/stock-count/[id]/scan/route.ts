...
export async function POST(req: NextRequest, { params }: RequestParam) {
    try {
        ...
        await requirePermission(ctx, PERMISSIONS.inventory.stockCount.count, { req: req });
        ...
        const parsed = scanItemSchema.safeParse(await req.json());
        if (!parsed.success) {
            return NextResponse.json(
        ...
        const data = await service.scanItem(
            ctx,
            idParsed.data.id,
            parsed.data.code,
        );
