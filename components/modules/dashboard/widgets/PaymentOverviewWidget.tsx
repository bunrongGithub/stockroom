...
import type { DashboardPayments } from '@/types/dashboard';

// Payment overview: period table + last-7-days single-series bar chart.
// Same template as SalesOverviewWidget; SKY hue because payments are a
// different entity than sales (dataviz: color follows the entity, and two
// money charts on one page must never share a hue).
...
export default function PaymentOverviewWidget({
    ...
    return (
        <section className="rounded-2xl border border-slate-100 bg-white p-5 shadow-sm">
            ...
            <table className="w-full font-mono text-xs tabular-nums">
                <thead>
                    <tr className="border-b text-[10px] uppercase tracking-wider text-slate-400">
                        ...
                        <th className="py-1.5 text-right font-bold">Received</th>
                        <th className="py-1.5 text-right font-bold">Payments</th>
                        <th className="py-1.5 text-right font-bold">Avg Payment</th>
                    </tr>
                </thead>
                <tbody>
                    {PERIODS.map(([key, label]) => {
                        const p = payments.periods[key];
            ...
            {/* Last 7 days — single-series bars (sky = payments entity) */}
