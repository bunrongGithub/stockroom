...
import { getOrder, cancelOrder, closeOrder, getDeliveryNotesByOrder } from '@/lib/mock-sales-store';
import type { SalesOrder, SalesOrderStatus, DeliveryNote } from '@/types/sales/order-management';
import { useEffect, useState } from 'react';
import { useParams, useRouter } from 'next/navigation';
import {
    ArrowLeftIcon, PencilIcon, TruckIcon, XCircleIcon, CheckCircleIcon,
...
function DNStatusBadge({ status }: { status: string }) {
    ...
    return (
        <span className={`inline-block rounded-full px-2 py-0.5 text-xs font-mono ${map[status] ?? 'bg-gray-100'}`}>
            {status}
        </span>
    );
}
...
export default function SaleOrderDetail({ currentPath, permission, currentPathActions }: ModuleProps) {
    ...
    function load() {
        const o = getOrder(id);
        setOrder(o);
        if (o) setDeliveryNotes(getDeliveryNotesByOrder(o.id));
        setLoading(false);
    }
    ...
    const canEdit = order.status === 'open' || order.status === 'partial_shipment';
    const canShip = order.status === 'open' || order.status === 'partial_shipment';
    const canCancel = order.status === 'open' || order.status === 'partial_shipment';
    const canClose = order.status === 'open' || order.status === 'partial_shipment';
    ...
    return (
        <div className="space-y-5">
                    ...
                    <div className="rounded-2xl bg-white p-6 shadow-xl w-80 space-y-4">
                        ...
                        <p className="text-xs text-muted-foreground">
                            {confirmAction === 'cancel'
                                ? 'This will void the order. Ensure there are no confirmed shipments first.'
                                : 'Closing the order will prevent further shipments.'}
                        </p>
                        <div className="flex justify-end gap-2">
                            <button onClick={() => setConfirmAction(null)} className="rounded-lg border px-3 py-1.5 text-xs hover:bg-muted font-mono">
                                Back
                            </button>
                            <button
                                onClick={confirmAction === 'cancel' ? handleCancel : handleClose}
                                className={`rounded-lg px-3 py-1.5 text-xs text-white font-mono ${confirmAction === 'cancel' ? 'bg-rose-500 hover:bg-rose-600' : 'bg-slate-600 hover:bg-slate-700'}`}
                            >
                                Confirm
                            </button>
                        </div>
                ...
                <div className="flex items-center gap-2">
                    {canEdit && (
                        <button
                            ...
                            className="inline-flex items-center gap-1.5 rounded-xl border border-violet-200 px-3 py-2 text-xs text-violet-600 hover:bg-violet-50 font-mono"
                        >
                    ...
                    {canShip && (
                        <button
                            onClick={() => {
                                if (typeof window !== 'undefined') sessionStorage.setItem('pending_dn_order_id', order.id);
                    ...
                    {canClose && (
                        <button
                            ...
                            className="inline-flex items-center gap-1.5 rounded-xl border border-slate-200 px-3 py-2 text-xs text-slate-600 hover:bg-slate-50 font-mono"
                        >
            ...
            <div className="grid grid-cols-2 gap-4">
                <Card className="border-none shadow-sm">
                    <CardHeader className="pb-2 pt-4 px-4">
                        <CardTitle className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Order Info</CardTitle>
                    </CardHeader>
                    <CardContent className="px-4 pb-4 grid grid-cols-2 gap-y-3 text-xs font-mono">
                        ...
                        <span className="text-muted-foreground">Warehouse</span><span>{order.warehouse}</span>
                        <span className="text-muted-foreground">Currency</span><span>{order.currency}</span>
                        {order.notes && (
                            <>
                                <span className="text-muted-foreground">Notes</span>
                                <span className="col-span-1">{order.notes}</span>
                            </>
                        )}
                    </CardContent>
                </Card>
                ...
                <Card className="border-none shadow-sm">
                    <CardHeader className="pb-2 pt-4 px-4">
                        <CardTitle className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Financials</CardTitle>
                    </CardHeader>
                    <CardContent className="px-4 pb-4 space-y-2 text-xs font-mono">
                        ...
                        <div className="flex justify-between">
                            ...
                            <span className="text-rose-500">- {fmt(order.discount_total)}</span>
                        </div>
                        <div className="flex justify-between">
                        ...
                        <div className="flex justify-between border-t pt-2 font-semibold text-sm">
                            ...
                            <span>{order.currency} {fmt(order.grand_total)}</span>
                        </div>
                    </CardContent>
                </Card>
            </div>
            ...
            <Card className="border-none shadow-sm">
                <CardHeader className="pb-2 pt-4 px-4">
                    <CardTitle className="text-xs font-semibold text-muted-foreground uppercase tracking-wide">Order Items</CardTitle>
                </CardHeader>
                                    ...
                                    return (
                                        <tr key={item.id} className="border-b hover:bg-muted/20">
                                            ...
                                            <td className="py-2 pr-3 text-right">
                                                <span className={remaining === 0 ? 'text-slate-400' : 'text-amber-600 font-medium'}>
                                                    {remaining}
                                                </span>
                                            </td>
                ...
                <Card className="border-none shadow-sm">
                    ...
                    <CardContent className="px-4 pb-4">
                        <table className="w-full text-xs font-mono">
                            <thead>
                                <tr className="border-b text-muted-foreground">
                                    <th className="text-left py-2 pr-3 font-medium">DN No</th>
                            ...
                            <tbody>
                                {deliveryNotes.map((dn) => (
                                    <tr key={dn.id} className="border-b hover:bg-muted/20">
                                        <td className="py-2 pr-3 font-semibold text-sky-600">
                                            <button onClick={() => router.push(`/sale/delivery-note/${dn.id}/view`)} className="hover:underline">
                                                {dn.delivery_no}
                                            </button>
                                        </td>
                                        <td className="py-2 pr-3">{dn.delivery_date}</td>
                                        <td className="py-2 pr-3">{dn.receiver_name || '—'}</td>
                                        <td className="py-2 pr-3"><DNStatusBadge status={dn.status} /></td>
                                        <td className="py-2">
                                            <button
            ...
            <div className="text-xs text-muted-foreground font-mono">
                Created by {order.created_by} • Last updated {new Date(order.updated_at).toLocaleString()}
            </div>
        </div>
    );
}
