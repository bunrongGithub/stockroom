  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        const data: WarehouseLocation[] = json.data ?? [];
        if (cancelled) return;
    ...
    try {
      ...
      if (movementType === 'out') {
        ...
        const balances: LocationBalance[] = Array.isArray(balanceJson)
          ? balanceJson
          : [];
        const balance = balances.find((row) => Number(row.item_id) === itemId);
      ...
      if (!res.ok)
        throw new Error(
          json.error?.message ?? json.error ?? 'Adjustment failed',
        );
      ...
      setError(
        err instanceof Error ? err.message : 'មាន​បញ្ហា​ក្នុង​ការ​រក្សា​ទុក។',
      );
    } finally {
      setSubmitting(false);
    }
